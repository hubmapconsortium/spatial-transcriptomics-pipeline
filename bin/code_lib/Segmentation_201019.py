...
# cellpose has to be installed in the environment, and the models have to be downloaded in cellpose's favorite directory: ~/.cellpose/models
    ...
    def segment(self, imgs, diameters=40, out_files=None, **kwargs):
        ...
        if diameters is None:
            ...
            for i, img in enumerate(imgs):
                mask, _, _, _ = self.base_model.eval(
                    [img], channels=[0, 0], diameter=diameters[i], **kwargs
                )
                if mask[0].max() < 2 ** 16 - 1:
                    initial_masks.append(mask[0].astype(np.uint16))
