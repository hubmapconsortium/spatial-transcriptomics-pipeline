...
class IJ_Stitch:
    def __init__(
        ...
        file_names="image_{ii}.tif",
        output_textfile_name="TileConfiguration.txt",
        fusion_method="Intensity of random input tile",
        regression_threshold=0.3,
        max_Over_avg_displacement_threshold=2.50,
        ...
        if "Grid" in Type:
            # a grid stitching
            self.typeArgs = self.prepGridStitching(
                Type,
                Order,
                grid_size_x,
                grid_size_y,
        ...
        self.commonArgs = self.prepCommons(
            fusion_method,
            regression_threshold,
            max_Over_avg_displacement_threshold,
            absolute_displacement_threshold,
            compute_overlap,
            invert_x_coordinates,
    ...
    def run(self):
        if self.macroname is None:
            dtn = datetime.now()
            macroFile = "{0}-{1}-{2}_{3}:{4}:{5}_{6}.ijm".format(
                dtn.year, dtn.month, dtn.day, dtn.hour, dtn.minute, dtn.second, self.type
            )
        ...
        if self.imagej_path.startswith("/"):
            ...
            shellCommand.append("/" + self.imagej_path)
        shellCommand = (
            shellCommand + ["--ij2"] + ["--headless"] + ["--console"] + ["-macro"] + [macroFile]
        )
    ...
    def assembleCommand(self):
        macro = """run("Grid/Collection stitching", "{}");""".format(
            self.typeArgs + " " + self.commonArgs
        )
        return macro

    def prepGridStitching(
        self,
        Type,
        Order,
        grid_size_x,
    ...
    def prepCommons(
        ...
        coms = (
            coms
            + " "
            + "max/avg_displacement_threshold={}".format(max_Over_avg_displacement_threshold)
        )
        coms = (
            coms
            + " "
            + "absolute_displacement_threshold={}".format(absolute_displacement_threshold)
        )
        if compute_overlap:
    ...
    def changeOutputName(self, output_dir, new_name=None):
        if new_name is None:
            new_name = re.sub(
                r"_[^_]+{[i]+}", "", self.file_names
            )  # removing _FOV{iii} from the name
    ...
    def getImageJ(pathOrFile):
        ...
        if os.path.isdir(pathOrFile):
            ...
            raise FileNotFoundError(
                "Invalid directory or file for ImageJ: \n {}".format(pathOrFile)
            )
